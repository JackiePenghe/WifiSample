...
public class WifiConnector {
    ...
    private void startConnect(Context context, String wifiSSid, String password, int tryCount) {
        ...
        if (wifiDevice == null) {
            if (tryCount > 3) {
                performWifiConnectFailedListener(wifiSSid);
                return;
            }
    ...
    private void showConnectWifiWithPassWord(final Context context, final WifiDevice wifiDevice) {
        final EditText editText = (EditText) View.inflate(context, R.layout.com_sscl_wifi_password_edit_text, null);
    ...
    private void showConnectWifiWithNameAndPassWord(final Context context, final EncryptWay encryptionWay) {
        final View view = View.inflate(context, R.layout.com_sscl_hidden_wifi_dialog, null);
    ...
    public boolean disConnect() {
        android.net.wifi.WifiManager systemWifiManager1 = WifiManager.getSystemWifiManager();
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            return systemWifiManager1.disconnect();
        } else {
            throw new RuntimeException("This method was deprecated in API level 29.\n a) See WifiNetworkSpecifier.Builder#build() for new mechanism to trigger connection to a Wi-Fi network.\n b) See addNetworkSuggestions(java.util.List), removeNetworkSuggestions(java.util.List) for new API to add Wi-Fi networks for consideration when auto-connecting to wifi. Compatibility Note: For applications targeting Build.VERSION_CODES.Q or above, this API will always return false");
        }
    }
